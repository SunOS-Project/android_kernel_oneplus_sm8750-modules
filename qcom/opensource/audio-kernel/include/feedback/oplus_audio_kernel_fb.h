...
#define audio_kernel_fb(level, delay, fmt, ...) \
	do { \
		...
		if (oplus_is_daemon_event_id(OPLUS_AUDIO_EVENTID_AUDIO_KERNEL_ERR)) { \
			char fb_info[MAX_PAYLOAD_DATASIZE] = {0}; \
			scnprintf(fb_info, MAX_PAYLOAD_DATASIZE - 1, "%s(%d):" fmt, __func__, __LINE__, ##__VA_ARGS__); \
			oplus_audio_daemon_feedback_event(OPLUS_AUDIO_EVENTID_AUDIO_KERNEL_ERR, fb_info); \
		} \
	} while (0)
#else /* CONFIG_AUDIO_DAEMON_KERNEL_QCOM */
#define audio_kernel_fb(level, delay, fmt, ...) \
	do { \
		static DEFINE_RATELIMIT_STATE(_rs, \
						FEEDBACK_RATELIMIT_INTERVAL, \
...
#define audio_kernel_fb_no_dm(level, delay, fmt, ...) \
	do { \
		static DEFINE_RATELIMIT_STATE(_rs, \
						FEEDBACK_RATELIMIT_INTERVAL, \
						FEEDBACK_RATELIMIT_BURST); \
...
/* replace "dev_err_ratelimited" and feedback */
#define dev_err_ratelimited_base_no_dm(level, delay, dev, fmt, ...) \
	do { \
		dev_level_ratelimited(dev_err, dev, fmt, ##__VA_ARGS__); \
		audio_kernel_fb_no_dm(level, delay, "payload@@%s %s:"dev_fmt(fmt), \
				dev_driver_string(dev), dev_name(dev), ##__VA_ARGS__); \
	} while (0)

#define dev_err_ratelimited_fb_no_dm(dev, fmt, ...) \
	dev_err_ratelimited_base_no_dm(FB_ERROR, 0, dev, fmt, ##__VA_ARGS__)

#define dev_err_ratelimited_fb_delay_no_dm(dev, fmt, ...) \
	dev_err_ratelimited_base_no_dm(FB_ERROR, FEEDBACK_DELAY_60S, dev, fmt, ##__VA_ARGS__)
